...
usage, yet these functions provided in this package can be generic to a variety of
other biological researches.

## Getting started
### Installation
```{r}
devtools::install_github("Koohoko/SynMut", quiet = TRUE)
library(SynMut)
```
...
### Generating synonymous sequences with maximal/minimal usage of specific codon
...
### Generating synonymous sequences with maximal/minimal usage of specific
dinucleotide 
...
### Generating synonymous sequences mimicing a specific codon usage pattern {#codon_mimic}

The function `codon_mimic` mutates the sequences to mimic a target codon usage 
patterns. Detail algorithm was provided at this [link](https://koohoko.github.io/SynMut/algorithm.html).
...
Output the DNA mutant sequcens
```{r, eval = FALSE}
