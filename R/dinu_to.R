...
dinu_to.not.keep <-
  function(seq.region,
    ...
    if (!is.na(max.dinu)) {
      ...
      codon.list.optm <- lapply(codon.list.alt, function(x) {
        filter1 <-
          x[!stringr::str_detect(x, pattern = min.dinu)] #difference
        if (length(filter1) == 1) {
          return(filter1)
        } else if (length(filter1) < 1) {
          filter2.1 <- x[!stringr::str_detect(x, #differnece
            pattern = paste0("^", substr(min.dinu, 2, 2)))]
          filter2.2 <- x[!stringr::str_detect(x, #differnece
...
dinu_to.keep <-
  function(check.region,
    ...
    if (check.region) {
      ...
      #regioned data
      seq.mut <- dinu_to.keep.region(codon.list.alt, seq, region,
        max.dinu, min.dinu)
    }
  ...
  for (i in seq_along(codon.list.alt)) {
    ...
    if (check.need) {
      ...
      codon.all.new <- mapply(function(x, y, z) {
        if (any(length(y) == 0, length(x) == 0)) {
          ...
          if (length(x) <= length(y)) {
            ...
            id.new.other <- which(!(seq_along(z) %in% id.new))
            codon.select <- z[x]
            codon.remainder <- z[-x]
            codon.all.new <- z
      ...
      seq.mut <- mapply(function(x, y, z) {
        z[y] <- x
        return(z)
      }, codon.all.new, pos.all, seq.mut, SIMPLIFY = FALSE)
    }
  }
  return(seq.mut)
}
